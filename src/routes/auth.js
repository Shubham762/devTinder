...
authRouter.post("/signup", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ message: "Error while saving data", error: err.message });
  }
});
...
authRouter.post("/login", async (req, res) => {
  try {
    ...
    if (isPasswordValid) {
      ...
      res
        .status(401)
        .send({ message: "Invalid password", error: "Invalid crdentials" });
    }
  } catch (err) {
    res
      .status(400)
      .send({ message: "Error while login with data", error: err.message });
  }
});
